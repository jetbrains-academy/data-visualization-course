...
class TestAddFunction(unittest.TestCase):
    test_cases: ClassVar[List[int]] = [2, 4, 20, 1900, 400, 2004, 2000, 2001]

    @staticmethod
    def correct_invoke_say_bye(how_many_times: int) -> str:
        return os.linesep.join([say_bye() for _ in range(how_many_times)])

    @patch("builtins.print")
    def test_parametrized(self, mock_print: Mock):
