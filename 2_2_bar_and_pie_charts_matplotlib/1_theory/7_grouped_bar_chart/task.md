...
As we can see, `bar` placed the first bar at 0, the second one at 1 and the last one at 2.
...
1. Use the hidden `aggregate` function to calculate the total sales (`sales`) over decades (`decade`) per region (`region`).
...
2. Implement the `plot_region` function. It should plot total region sales over decades like in the example.
   ...
   and the `get_region_sales` for getting sales data for a specified region only.
...
<div class="hint" title="How to aggregate the data?">
   To calculate the total sales over decades per region:
   <ol>
      <li>Use the <a href="https://pandas.pydata.org/docs/reference/api/pandas.cut.html"><code>cut</code></a> function to convert a year of release to a decade.</li>
      ...
      and calculate the total sales using the <a href="https://pandas.pydata.org/docs/reference/api/pandas.core.groupby.DataFrameGroupBy.sum.html"><code>sum</code></a> function.</li>
   </ol>
