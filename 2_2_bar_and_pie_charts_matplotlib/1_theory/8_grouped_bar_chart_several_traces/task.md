...
So far, we've only plotted sales for one region, so let's add more!

## Task

Use the hidden `get_all_regions` function, which returns all sales regions.
Iterate over the result and plot sales for each region using the `plot_region` function.
