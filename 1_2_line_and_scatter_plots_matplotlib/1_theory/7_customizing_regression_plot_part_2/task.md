...
Also, we can change how ticks are distributed. To manually set ticks, we can pass the desirable tick positions to
...
To make them disappear, we should call the `set_visible` method with argument `False`.

## Task

1. Change x-label to `User Score` and y-label to `Critic Score`
2. Set the ticks on the x-axis from `0` to `10` without skips. 
3. Set the ticks on the y-axis from `0` to `100` on every `10`.
