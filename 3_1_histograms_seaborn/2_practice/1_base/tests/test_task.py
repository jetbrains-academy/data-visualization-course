...
class PlotTestCase(HistTestMixin, AxisTestMixin, LegendTestMixin):
    ...
    def test_2_2_bar_height(self):
        for container_number, city in enumerate(self.publishers):
            filtered_dataset = self.data[self.data["city"] == city]
            weights = np.ones_like(filtered_dataset["sales"]) / filtered_dataset.shape[0]
            counts, _ = np.histogram(filtered_dataset["sales"], bins=self.bins, weights=weights)
