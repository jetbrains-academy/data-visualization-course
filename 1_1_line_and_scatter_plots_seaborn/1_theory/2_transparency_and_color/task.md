...
Now we can see how the scores are distributed,
but we can't understand the density of points because they are too packed.
To understand the density better, we can adjust the plot's transparency.

In seaborn, you can pass the `alpha` parameter to functions to control this.
It accepts a float number between 0 and 1.

Also, let's familiarize ourselves with another helpful parameter, `color`,
which is used to change the color of the plotted points, lines and shapes.
It can accept many types of inputs:
